            ...
            <div class="col-md-4 text-start">
                ...
                @if ($user)
                    @if ($user->role->role_name === 'teacher')
                        <a href="{{ route('teacher.my-courses') }}" class="btn btn-outline-secondary mb-2">
                            <i class="fas fa-arrow-right me-2"></i>رجوع للكورسات
                        </a>
                    @elseif($user->role->role_name === 'student')
                        <a href="{{ route('student.courses') }}" class="btn btn-outline-secondary mb-2">
                            <i class="fas fa-arrow-right me-2"></i>رجوع للكورسات
                        </a>
                    @else
                        ...
                        <div class="lesson-header d-flex justify-content-between align-items-center">
                            ...
                            @if (auth()->check() && auth()->user()->role->role_name === 'teacher')
                                <div class="btn-group">
                                    ...
                                    <form action="{{ route('lessons.start', $lesson->id) }}" method="POST"
                                        class="d-inline ms-1">
                                        @csrf
                                        <button type="submit" class="btn btn-sm btn-success"
                                            onclick="return confirm('هل تريد بدء هذا الدرس الآن؟')">
